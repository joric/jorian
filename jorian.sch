EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr B 17000 11000
encoding utf-8
Sheet 1 1
Title "jorian"
Date "22 07 2017"
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L Connector_Generic:Conn_01x04 E16
U 1 1 5C28A7CD
P 10300 7075
F 0 "E16" H 10300 7275 50  0000 C CNN
F 1 "right" H 10300 6775 50  0000 C CNN
F 2 "kbd:edge_conn" H 10300 7075 50  0001 C CNN
F 3 "" H 10300 7075 50  0001 C CNN
	1    10300 7075
	-1   0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x04 E18
U 1 1 5C28AA80
P 10100 7075
F 0 "E18" H 10100 7275 50  0000 C CNN
F 1 "left" H 10100 6775 50  0000 C CNN
F 2 "kbd:edge_conn" H 10100 7075 50  0001 C CNN
F 3 "" H 10100 7075 50  0001 C CNN
	1    10100 7075
	1    0    0    -1  
$EndComp
Text Label 10500 7175 0    50   ~ 0
GND
Text Label 10500 7275 0    50   ~ 0
S18
Text Label 10500 6975 0    50   ~ 0
LED_VCC
$Comp
L Connector_Generic:Conn_01x04 E19
U 1 1 5C6E087C
P 11800 5650
F 0 "E19" H 11800 5850 50  0000 C CNN
F 1 "right" H 11800 5350 50  0000 C CNN
F 2 "kbd:edge_conn" H 11800 5650 50  0001 C CNN
F 3 "" H 11800 5650 50  0001 C CNN
	1    11800 5650
	-1   0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x04 E5
U 1 1 5C6E0883
P 11600 5650
F 0 "E5" H 11600 5850 50  0000 C CNN
F 1 "left" H 11600 5350 50  0000 C CNN
F 2 "kbd:edge_conn" H 11600 5650 50  0001 C CNN
F 3 "" H 11600 5650 50  0001 C CNN
	1    11600 5650
	1    0    0    -1  
$EndComp
Text Label 11400 5850 2    50   ~ 0
S19
Text Label 11400 6550 2    50   ~ 0
S20
$Comp
L Connector_Generic:Conn_01x04 E21
U 1 1 5C6F2906
P 11800 7050
F 0 "E21" H 11800 7250 50  0000 C CNN
F 1 "right" H 11800 6750 50  0000 C CNN
F 2 "kbd:edge_conn" H 11800 7050 50  0001 C CNN
F 3 "" H 11800 7050 50  0001 C CNN
	1    11800 7050
	-1   0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x04 E15
U 1 1 5C6F290D
P 11600 7050
F 0 "E15" H 11600 7250 50  0000 C CNN
F 1 "left" H 11600 6750 50  0000 C CNN
F 2 "kbd:edge_conn" H 11600 7050 50  0001 C CNN
F 3 "" H 11600 7050 50  0001 C CNN
	1    11600 7050
	1    0    0    -1  
$EndComp
Text Label 11400 6350 2    50   ~ 0
S22
Text Label 10500 7075 0    50   ~ 0
DIN18
Text Label 9900 7275 2    50   ~ 0
S18_A
Text Label 9900 7175 2    50   ~ 0
GND_A
Text Label 9900 6975 2    50   ~ 0
VCC_A
Text Label 9900 7075 2    50   ~ 0
DIN18_A
Text Label 12000 5850 0    50   ~ 0
S19_B
Text Label 12000 6550 0    50   ~ 0
S20_B
Text Label 12000 7250 0    50   ~ 0
S21_B
Text Label 12000 6350 0    50   ~ 0
S22_B
Text Label 12000 7050 0    50   ~ 0
VCC_B
Text Label 12000 5650 0    50   ~ 0
DIN21_B
Text Label 11400 5550 2    50   ~ 0
DIN5
Text Label 12000 5550 0    50   ~ 0
DIN5_B
Text Label 11400 5650 2    50   ~ 0
DIN21
$Comp
L Device:Battery_Cell BT1
U 1 1 5C194810
P 8700 9850
F 0 "BT1" H 8583 9946 50  0000 R CNN
F 1 "Battery Holder" H 8583 9855 50  0000 R CNN
F 2 "kbd:cr2032_bs-7" V 8700 9910 50  0001 C CNN
F 3 "~" V 8700 9910 50  0001 C CNN
	1    8700 9850
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x04 E20
U 1 1 5C1C4E7E
P 11800 6350
F 0 "E20" H 11800 6550 50  0000 C CNN
F 1 "right" H 11800 6050 50  0000 C CNN
F 2 "kbd:edge_conn" H 11800 6350 50  0001 C CNN
F 3 "" H 11800 6350 50  0001 C CNN
	1    11800 6350
	-1   0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x04 E10
U 1 1 5C1C4E85
P 11600 6350
F 0 "E10" H 11600 6550 50  0000 C CNN
F 1 "left" H 11600 6050 50  0000 C CNN
F 2 "kbd:edge_conn" H 11600 6350 50  0001 C CNN
F 3 "" H 11600 6350 50  0001 C CNN
	1    11600 6350
	1    0    0    -1  
$EndComp
Wire Wire Line
	4700 1750 4700 2350
Wire Wire Line
	4700 4750 6100 4750
Wire Wire Line
	10300 4750 10300 2950
Wire Wire Line
	6100 1750 6100 2350
Wire Wire Line
	7500 1750 7500 2350
Wire Wire Line
	8900 1750 8900 2350
Text Label 5500 1750 0    70   ~ 0
S1
Text Label 5500 2350 0    70   ~ 0
S6
Text Label 5500 2950 0    70   ~ 0
S11
Text Label 5500 3550 0    70   ~ 0
S16
Text Label 5650 4150 0    70   ~ 0
S20_B
Text Label 6900 1750 0    70   ~ 0
S2
Text Label 6900 2350 0    70   ~ 0
S7
Text Label 6900 2950 0    70   ~ 0
S12
Text Label 6900 3550 0    70   ~ 0
S17
Text Label 7025 4150 0    70   ~ 0
S21_B
Text Label 8300 1750 0    70   ~ 0
S3
Text Label 8300 2350 0    70   ~ 0
S8
Text Label 8300 2950 0    70   ~ 0
S13
Text Label 8500 3575 0    70   ~ 0
S18_A
Text Label 8500 4150 0    70   ~ 0
S22_B
Text Label 9700 1750 0    70   ~ 0
S4
Text Label 9700 2350 0    70   ~ 0
S9
Text Label 9700 2950 0    70   ~ 0
S14
Text Label 9850 3500 0    70   ~ 0
S19_B
Text Label 11100 1750 0    70   ~ 0
S5
Text Label 11100 2350 0    70   ~ 0
S10
Text Label 11100 2950 0    70   ~ 0
S15
Wire Wire Line
	5700 1750 5400 1750
Wire Wire Line
	5700 2350 5400 2350
Wire Wire Line
	5700 2950 5400 2950
Wire Wire Line
	5700 3550 5400 3550
Wire Wire Line
	5850 4150 5550 4150
Wire Wire Line
	7100 1750 6800 1750
Wire Wire Line
	7100 2350 6800 2350
Wire Wire Line
	7100 2950 6800 2950
Wire Wire Line
	7100 3550 6800 3550
Wire Wire Line
	7225 4150 6925 4150
Wire Wire Line
	8500 1750 8200 1750
Wire Wire Line
	8500 2350 8200 2350
Wire Wire Line
	8500 2950 8200 2950
Wire Wire Line
	8700 3575 8400 3575
Wire Wire Line
	9900 1750 9600 1750
Wire Wire Line
	9900 2350 9600 2350
Wire Wire Line
	9900 2950 9600 2950
Wire Wire Line
	11300 1750 11000 1750
Wire Wire Line
	11300 2350 11000 2350
Wire Wire Line
	11300 2950 11000 2950
Connection ~ 4700 3550
Connection ~ 4700 2950
Connection ~ 4700 2350
Connection ~ 6100 4750
Connection ~ 6100 3550
Connection ~ 6100 2950
Connection ~ 6100 2350
Connection ~ 7500 4750
Connection ~ 7500 2950
Connection ~ 7500 2350
Connection ~ 8900 4750
Connection ~ 8900 2950
Connection ~ 8900 2350
Connection ~ 10300 2950
Connection ~ 10300 2350
$Comp
L kbd:CHERRY_MX S1
U 1 1 5C18CE44
P 4900 1650
F 0 "S1" H 4950 1700 60  0000 L BNN
F 1 "CHERRY MX" H 4900 1450 60  0000 L BNN
F 2 "kbd:cherry" H 9090 5720 60  0001 C CNN
F 3 "" H 9090 5720 60  0000 C CNN
	1    4900 1650
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S21
U 1 1 5C18CE4B
P 6425 4050
F 0 "S21" H 6475 4100 60  0000 L BNN
F 1 "CHERRY MX" H 6425 3850 60  0000 L BNN
F 2 "kbd:cherry" H 12015 10520 60  0001 C CNN
F 3 "" H 12015 10520 60  0000 C CNN
	1    6425 4050
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S3
U 1 1 5C18CE52
P 7700 1650
F 0 "S3" H 7750 1700 60  0000 L BNN
F 1 "CHERRY MX" H 7700 1450 60  0000 L BNN
F 2 "kbd:cherry" H 14690 5720 60  0001 C CNN
F 3 "" H 14690 5720 60  0000 C CNN
	1    7700 1650
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S8
U 1 1 5C18CE59
P 7700 2250
F 0 "S8" H 7750 2300 60  0000 L BNN
F 1 "CHERRY MX" H 7700 2050 60  0000 L BNN
F 2 "kbd:cherry" H 14690 6920 60  0001 C CNN
F 3 "" H 14690 6920 60  0000 C CNN
	1    7700 2250
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S13
U 1 1 5C18CE60
P 7700 2850
F 0 "S13" H 7750 2900 60  0000 L BNN
F 1 "CHERRY MX" H 7700 2650 60  0000 L BNN
F 2 "kbd:cherry" H 14690 8120 60  0001 C CNN
F 3 "" H 14690 8120 60  0000 C CNN
	1    7700 2850
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S18
U 1 1 5C18CE67
P 7900 3475
F 0 "S18" H 7950 3525 60  0000 L BNN
F 1 "CHERRY MX" H 7900 3275 60  0000 L BNN
F 2 "kbd:cherry" H 14890 9345 60  0001 C CNN
F 3 "" H 14890 9345 60  0000 C CNN
	1    7900 3475
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S22
U 1 1 5C18CE6E
P 7900 4050
F 0 "S22" H 7950 4100 60  0000 L BNN
F 1 "CHERRY MX" H 7900 3850 60  0000 L BNN
F 2 "kbd:cherry" H 14890 10520 60  0001 C CNN
F 3 "" H 14890 10520 60  0000 C CNN
	1    7900 4050
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S4
U 1 1 5C18CE75
P 9100 1650
F 0 "S4" H 9150 1700 60  0000 L BNN
F 1 "CHERRY MX" H 9100 1450 60  0000 L BNN
F 2 "kbd:cherry" H 17490 5720 60  0001 C CNN
F 3 "" H 17490 5720 60  0000 C CNN
	1    9100 1650
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S9
U 1 1 5C18CE7C
P 9100 2250
F 0 "S9" H 9150 2300 60  0000 L BNN
F 1 "CHERRY MX" H 9100 2050 60  0000 L BNN
F 2 "kbd:cherry" H 17490 6920 60  0001 C CNN
F 3 "" H 17490 6920 60  0000 C CNN
	1    9100 2250
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S14
U 1 1 5C18CE83
P 9100 2850
F 0 "S14" H 9150 2900 60  0000 L BNN
F 1 "CHERRY MX" H 9100 2650 60  0000 L BNN
F 2 "kbd:cherry" H 17490 8120 60  0001 C CNN
F 3 "" H 17490 8120 60  0000 C CNN
	1    9100 2850
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S19
U 1 1 5C18CE8A
P 9250 3400
F 0 "S19" H 9300 3450 60  0000 L BNN
F 1 "CHERRY MX" H 9250 3200 60  0000 L BNN
F 2 "kbd:cherry" H 17640 9270 60  0001 C CNN
F 3 "" H 17640 9270 60  0000 C CNN
	1    9250 3400
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S6
U 1 1 5C18CE91
P 4900 2250
F 0 "S6" H 4950 2300 60  0000 L BNN
F 1 "CHERRY MX" H 4900 2050 60  0000 L BNN
F 2 "kbd:cherry" H 9090 6920 60  0001 C CNN
F 3 "" H 9090 6920 60  0000 C CNN
	1    4900 2250
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S5
U 1 1 5C18CE9F
P 10500 1650
F 0 "S5" H 10550 1700 60  0000 L BNN
F 1 "CHERRY MX" H 10500 1450 60  0000 L BNN
F 2 "kbd:cherry" H 20290 5720 60  0001 C CNN
F 3 "" H 20290 5720 60  0000 C CNN
	1    10500 1650
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S10
U 1 1 5C18CEA6
P 10500 2250
F 0 "S10" H 10550 2300 60  0000 L BNN
F 1 "CHERRY MX" H 10500 2050 60  0000 L BNN
F 2 "kbd:cherry" H 20290 6920 60  0001 C CNN
F 3 "" H 20290 6920 60  0000 C CNN
	1    10500 2250
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S15
U 1 1 5C18CEAD
P 10500 2850
F 0 "S15" H 10550 2900 60  0000 L BNN
F 1 "CHERRY MX" H 10500 2650 60  0000 L BNN
F 2 "kbd:cherry" H 20290 8120 60  0001 C CNN
F 3 "" H 20290 8120 60  0000 C CNN
	1    10500 2850
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S11
U 1 1 5C18CEB4
P 4900 2850
F 0 "S11" H 4950 2900 60  0000 L BNN
F 1 "CHERRY MX" H 4900 2650 60  0000 L BNN
F 2 "kbd:cherry" H 9090 8120 60  0001 C CNN
F 3 "" H 9090 8120 60  0000 C CNN
	1    4900 2850
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S16
U 1 1 5C18CEBB
P 4900 3450
F 0 "S16" H 4950 3500 60  0000 L BNN
F 1 "CHERRY MX" H 4900 3250 60  0000 L BNN
F 2 "kbd:cherry" H 9090 9320 60  0001 C CNN
F 3 "" H 9090 9320 60  0000 C CNN
	1    4900 3450
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S20
U 1 1 5C18CEC2
P 5050 4050
F 0 "S20" H 5100 4100 60  0000 L BNN
F 1 "CHERRY MX" H 5050 3850 60  0000 L BNN
F 2 "kbd:cherry" H 9240 10520 60  0001 C CNN
F 3 "" H 9240 10520 60  0000 C CNN
	1    5050 4050
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S2
U 1 1 5C18CEC9
P 6300 1650
F 0 "S2" H 6350 1700 60  0000 L BNN
F 1 "CHERRY MX" H 6300 1450 60  0000 L BNN
F 2 "kbd:cherry" H 11890 5720 60  0001 C CNN
F 3 "" H 11890 5720 60  0000 C CNN
	1    6300 1650
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S7
U 1 1 5C18CED0
P 6300 2250
F 0 "S7" H 6350 2300 60  0000 L BNN
F 1 "CHERRY MX" H 6300 2050 60  0000 L BNN
F 2 "kbd:cherry" H 11890 6920 60  0001 C CNN
F 3 "" H 11890 6920 60  0000 C CNN
	1    6300 2250
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S12
U 1 1 5C18CED7
P 6300 2850
F 0 "S12" H 6350 2900 60  0000 L BNN
F 1 "CHERRY MX" H 6300 2650 60  0000 L BNN
F 2 "kbd:cherry" H 11890 8120 60  0001 C CNN
F 3 "" H 11890 8120 60  0000 C CNN
	1    6300 2850
	1    0    0    -1  
$EndComp
$Comp
L kbd:CHERRY_MX S17
U 1 1 5C18CEDE
P 6300 3450
F 0 "S17" H 6350 3500 60  0000 L BNN
F 1 "CHERRY MX" H 6300 3250 60  0000 L BNN
F 2 "kbd:cherry" H 11890 9320 60  0001 C CNN
F 3 "" H 11890 9320 60  0000 C CNN
	1    6300 3450
	1    0    0    -1  
$EndComp
Wire Wire Line
	4700 2950 4700 3550
Wire Wire Line
	4700 2350 4700 2950
Wire Wire Line
	6100 4750 7500 4750
Wire Wire Line
	6100 2950 6100 3550
Wire Wire Line
	6100 2350 6100 2950
Wire Wire Line
	7500 4750 8900 4750
Wire Wire Line
	7500 2350 7500 2950
Wire Wire Line
	8900 4750 10300 4750
Wire Wire Line
	8900 2350 8900 2950
Wire Wire Line
	10300 2950 10300 2350
Wire Wire Line
	10300 2350 10300 1750
Wire Wire Line
	4700 7475 4950 7475
Wire Wire Line
	4700 5150 4700 7475
Wire Wire Line
	5450 7475 5450 7625
Connection ~ 5450 7475
Wire Wire Line
	5350 7475 5450 7475
Wire Wire Line
	5450 7275 5450 7475
Connection ~ 5450 7275
Wire Wire Line
	5550 7275 5450 7275
Wire Wire Line
	5450 6975 5450 7275
Connection ~ 5450 6975
Wire Wire Line
	5350 6975 5450 6975
Wire Wire Line
	5450 6775 5450 6975
Connection ~ 5450 6775
Wire Wire Line
	5550 6775 5450 6775
Wire Wire Line
	5450 6475 5450 6775
Connection ~ 5450 6475
Wire Wire Line
	5350 6475 5450 6475
Wire Wire Line
	5450 6275 5450 6475
Wire Wire Line
	5450 5975 5450 6275
Connection ~ 5450 6275
Wire Wire Line
	5550 6275 5450 6275
Wire Wire Line
	5350 5975 5450 5975
Wire Wire Line
	5550 5775 5450 5775
Connection ~ 5450 5975
Wire Wire Line
	5450 5775 5450 5975
Connection ~ 6050 5975
Wire Wire Line
	6050 5975 5950 5975
Connection ~ 6050 6475
Wire Wire Line
	6050 6475 5950 6475
Connection ~ 6050 6975
Wire Wire Line
	6050 6975 5950 6975
Wire Wire Line
	6050 7475 5950 7475
Wire Wire Line
	4850 6775 4850 7275
Connection ~ 4850 6775
Wire Wire Line
	4950 6775 4850 6775
Wire Wire Line
	4850 6275 4850 6775
Wire Wire Line
	4850 5775 4850 6275
Connection ~ 4850 6275
Wire Wire Line
	4950 6275 4850 6275
Wire Wire Line
	4950 5775 4850 5775
Connection ~ 4850 5775
Wire Wire Line
	4850 5625 4850 5775
Wire Wire Line
	4850 7275 4950 7275
Wire Wire Line
	5550 7475 5550 7625
Wire Wire Line
	5950 5625 5950 5775
Wire Wire Line
	5350 5625 5950 5625
Wire Wire Line
	5350 5775 5350 5625
Wire Wire Line
	5550 7125 5550 6975
Wire Wire Line
	5950 7125 5550 7125
Wire Wire Line
	5950 7275 5950 7125
Wire Wire Line
	5550 6625 5550 6475
Wire Wire Line
	5950 6625 5550 6625
Wire Wire Line
	5950 6775 5950 6625
Wire Wire Line
	5550 6125 5550 5975
Wire Wire Line
	5950 6125 5550 6125
Wire Wire Line
	5950 6275 5950 6125
Wire Wire Line
	4950 6125 4950 5975
Wire Wire Line
	5350 6125 4950 6125
Wire Wire Line
	5350 6275 5350 6125
Wire Wire Line
	4950 6625 4950 6475
Wire Wire Line
	5350 6625 4950 6625
Wire Wire Line
	5350 6775 5350 6625
Wire Wire Line
	4950 7125 4950 6975
Wire Wire Line
	5350 7125 4950 7125
Wire Wire Line
	5350 7275 5350 7125
$Comp
L kbd:kbd_SK6812MINI L17
U 1 1 5C19B0B6
P 5150 7375
F 0 "L17" H 5150 7375 59  0000 C CNN
F 1 "SK6812MINI" H 5150 7325 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 5150 7375 60  0001 C CNN
F 3 "" H 5150 7375 60  0001 C CNN
	1    5150 7375
	-1   0    0    -1  
$EndComp
$Comp
L kbd:kbd_SK6812MINI L10
U 1 1 5C19B0AF
P 5750 7375
F 0 "L10" H 5750 7375 59  0000 C CNN
F 1 "SK6812MINI" H 5750 7325 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 5750 7375 60  0001 C CNN
F 3 "" H 5750 7375 60  0001 C CNN
	1    5750 7375
	1    0    0    1   
$EndComp
$Comp
L kbd:kbd_SK6812MINI L12
U 1 1 5C19B0A1
P 5150 6875
F 0 "L12" H 5150 6875 59  0000 C CNN
F 1 "SK6812MINI" H 5150 6825 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 5150 6875 60  0001 C CNN
F 3 "" H 5150 6875 60  0001 C CNN
	1    5150 6875
	-1   0    0    -1  
$EndComp
$Comp
L kbd:kbd_SK6812MINI L9
U 1 1 5C19B09A
P 5750 6875
F 0 "L9" H 5750 6875 59  0000 C CNN
F 1 "SK6812MINI" H 5750 6825 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 5750 6875 60  0001 C CNN
F 3 "" H 5750 6875 60  0001 C CNN
	1    5750 6875
	1    0    0    1   
$EndComp
$Comp
L kbd:kbd_SK6812MINI L7
U 1 1 5C19B077
P 5150 6375
F 0 "L7" H 5150 6375 59  0000 C CNN
F 1 "SK6812MINI" H 5150 6325 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 5150 6375 60  0001 C CNN
F 3 "" H 5150 6375 60  0001 C CNN
	1    5150 6375
	-1   0    0    -1  
$EndComp
$Comp
L kbd:kbd_SK6812MINI L14
U 1 1 5C19B070
P 5750 6375
F 0 "L14" H 5750 6375 59  0000 C CNN
F 1 "SK6812MINI" H 5750 6325 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 5750 6375 60  0001 C CNN
F 3 "" H 5750 6375 60  0001 C CNN
	1    5750 6375
	1    0    0    1   
$EndComp
$Comp
L kbd:kbd_SK6812MINI L8
U 1 1 5C19B04D
P 5150 5875
F 0 "L8" H 5150 5875 59  0000 C CNN
F 1 "SK6812MINI" H 5150 5825 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 5150 5875 60  0001 C CNN
F 3 "" H 5150 5875 60  0001 C CNN
	1    5150 5875
	-1   0    0    -1  
$EndComp
$Comp
L kbd:kbd_SK6812MINI L13
U 1 1 5C19B046
P 5750 5875
F 0 "L13" H 5750 5875 59  0000 C CNN
F 1 "SK6812MINI" H 5750 5825 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 5750 5875 60  0001 C CNN
F 3 "" H 5750 5875 60  0001 C CNN
	1    5750 5875
	1    0    0    1   
$EndComp
Wire Wire Line
	6050 5625 6050 5975
Wire Wire Line
	6050 5975 6050 6475
Wire Wire Line
	6050 6475 6050 6975
Wire Wire Line
	6900 7475 7000 7475
Wire Wire Line
	7500 7475 7500 7700
Connection ~ 7500 7475
Wire Wire Line
	7400 7475 7500 7475
Connection ~ 7500 6975
Wire Wire Line
	7400 6975 7500 6975
Connection ~ 7500 6475
Wire Wire Line
	7400 6475 7500 6475
Wire Wire Line
	7400 5975 7500 5975
Wire Wire Line
	6900 6775 6900 7275
Connection ~ 6900 6775
Wire Wire Line
	7000 6775 6900 6775
Wire Wire Line
	6900 6275 6900 6775
Wire Wire Line
	6900 5775 6900 6275
Connection ~ 6900 6275
Wire Wire Line
	7000 6275 6900 6275
Wire Wire Line
	7000 5775 6900 5775
Connection ~ 6900 5775
Wire Wire Line
	6900 5625 6900 5775
Wire Wire Line
	6900 7275 7000 7275
Wire Wire Line
	7400 5775 7500 5775
Wire Wire Line
	7000 6125 7000 5975
Wire Wire Line
	7400 6125 7000 6125
Wire Wire Line
	7400 6275 7400 6125
Wire Wire Line
	7000 6625 7000 6475
Wire Wire Line
	7400 6625 7000 6625
Wire Wire Line
	7400 6775 7400 6625
Wire Wire Line
	7000 7125 7000 6975
Wire Wire Line
	7400 7125 7000 7125
Wire Wire Line
	7400 7275 7400 7125
$Comp
L kbd:kbd_SK6812MINI L21
U 1 1 5C2E54AD
P 7200 7375
F 0 "L21" H 7200 7375 59  0000 C CNN
F 1 "SK6812MINI" H 7200 7325 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 7200 7375 60  0001 C CNN
F 3 "" H 7200 7375 60  0001 C CNN
	1    7200 7375
	-1   0    0    -1  
$EndComp
$Comp
L kbd:kbd_SK6812MINI L20
U 1 1 5C2E54B4
P 7200 6875
F 0 "L20" H 7200 6875 59  0000 C CNN
F 1 "SK6812MINI" H 7200 6825 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 7200 6875 60  0001 C CNN
F 3 "" H 7200 6875 60  0001 C CNN
	1    7200 6875
	-1   0    0    -1  
$EndComp
$Comp
L kbd:kbd_SK6812MINI L22
U 1 1 5C2E54BB
P 7200 6375
F 0 "L22" H 7200 6375 59  0000 C CNN
F 1 "SK6812MINI" H 7200 6325 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 7200 6375 60  0001 C CNN
F 3 "" H 7200 6375 60  0001 C CNN
	1    7200 6375
	-1   0    0    -1  
$EndComp
$Comp
L kbd:kbd_SK6812MINI L19
U 1 1 5C2E54C2
P 7200 5875
F 0 "L19" H 7200 5875 59  0000 C CNN
F 1 "SK6812MINI" H 7200 5825 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 7200 5875 60  0001 C CNN
F 3 "" H 7200 5875 60  0001 C CNN
	1    7200 5875
	-1   0    0    -1  
$EndComp
Text Label 6900 7700 0    50   ~ 0
DIN21_B
Text Label 6900 5625 0    50   ~ 0
VCC_B
Text Label 7500 7700 2    50   ~ 0
GND_B
Wire Wire Line
	7500 6975 7500 7475
Wire Wire Line
	7500 6475 7500 6975
Wire Wire Line
	7500 5975 7500 6475
Wire Wire Line
	7500 5775 7500 5625
Text Label 7500 5625 2    50   ~ 0
DIN5_B
Wire Wire Line
	6900 7475 6900 7700
Wire Wire Line
	5550 7625 6150 7625
Wire Wire Line
	6150 7625 6150 7475
Wire Wire Line
	6050 6975 6050 7275
Wire Wire Line
	6050 7275 6050 7475
Connection ~ 6050 7275
Wire Wire Line
	6150 7275 6050 7275
Text Label 6550 7275 0    50   ~ 0
DIN21
Wire Wire Line
	6650 7475 6650 7625
Wire Wire Line
	6550 7475 6650 7475
$Comp
L kbd:kbd_SK6812MINI L15
U 1 1 5C19B0BD
P 6350 7375
F 0 "L15" H 6350 7375 59  0000 C CNN
F 1 "SK6812MINI" H 6350 7325 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 6350 7375 60  0001 C CNN
F 3 "" H 6350 7375 60  0001 C CNN
	1    6350 7375
	-1   0    0    -1  
$EndComp
Wire Wire Line
	7850 7475 8100 7475
Wire Wire Line
	7850 5150 7850 7475
Wire Wire Line
	8600 7475 8600 7625
Connection ~ 8600 7475
Wire Wire Line
	8500 7475 8600 7475
Wire Wire Line
	8600 7275 8600 7475
Connection ~ 8600 7275
Wire Wire Line
	8700 7275 8600 7275
Wire Wire Line
	8600 6975 8600 7275
Connection ~ 8600 6975
Wire Wire Line
	8500 6975 8600 6975
Wire Wire Line
	8600 6775 8600 6975
Connection ~ 8600 6775
Wire Wire Line
	8700 6775 8600 6775
Wire Wire Line
	8600 6475 8600 6775
Connection ~ 8600 6475
Wire Wire Line
	8500 6475 8600 6475
Wire Wire Line
	8600 6275 8600 6475
Wire Wire Line
	8600 5975 8600 6275
Connection ~ 8600 6275
Wire Wire Line
	8700 6275 8600 6275
Wire Wire Line
	8500 5975 8600 5975
Wire Wire Line
	8700 5775 8600 5775
Connection ~ 8600 5975
Wire Wire Line
	8600 5775 8600 5975
Connection ~ 9200 5975
Wire Wire Line
	9200 5975 9100 5975
Connection ~ 9200 6475
Wire Wire Line
	9200 6475 9100 6475
Connection ~ 9200 6975
Wire Wire Line
	9200 6975 9100 6975
Wire Wire Line
	9200 7475 9100 7475
Wire Wire Line
	8000 6775 8000 7275
Connection ~ 8000 6775
Wire Wire Line
	8100 6775 8000 6775
Wire Wire Line
	8000 6275 8000 6775
Wire Wire Line
	8000 5775 8000 6275
Connection ~ 8000 6275
Wire Wire Line
	8100 6275 8000 6275
Wire Wire Line
	8100 5775 8000 5775
Connection ~ 8000 5775
Wire Wire Line
	8000 5625 8000 5775
Wire Wire Line
	8000 7275 8100 7275
Wire Wire Line
	8700 7475 8700 7625
Wire Wire Line
	9100 5625 9100 5775
Wire Wire Line
	8500 5625 9100 5625
Wire Wire Line
	8500 5775 8500 5625
Wire Wire Line
	8700 7125 8700 6975
Wire Wire Line
	9100 7125 8700 7125
Wire Wire Line
	9100 7275 9100 7125
Wire Wire Line
	8700 6625 8700 6475
Wire Wire Line
	9100 6625 8700 6625
Wire Wire Line
	9100 6775 9100 6625
Wire Wire Line
	8700 6125 8700 5975
Wire Wire Line
	9100 6125 8700 6125
Wire Wire Line
	9100 6275 9100 6125
Wire Wire Line
	8100 6125 8100 5975
Wire Wire Line
	8500 6125 8100 6125
Wire Wire Line
	8500 6275 8500 6125
Wire Wire Line
	8100 6625 8100 6475
Wire Wire Line
	8500 6625 8100 6625
Wire Wire Line
	8500 6775 8500 6625
Wire Wire Line
	8100 7125 8100 6975
Wire Wire Line
	8500 7125 8100 7125
Wire Wire Line
	8500 7275 8500 7125
$Comp
L kbd:kbd_SK6812MINI L5
U 1 1 5C50902D
P 8300 7375
F 0 "L5" H 8300 7375 59  0000 C CNN
F 1 "SK6812MINI" H 8300 7325 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 8300 7375 60  0001 C CNN
F 3 "" H 8300 7375 60  0001 C CNN
	1    8300 7375
	-1   0    0    -1  
$EndComp
$Comp
L kbd:kbd_SK6812MINI L16
U 1 1 5C509034
P 8900 7375
F 0 "L16" H 8900 7375 59  0000 C CNN
F 1 "SK6812MINI" H 8900 7325 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 8900 7375 60  0001 C CNN
F 3 "" H 8900 7375 60  0001 C CNN
	1    8900 7375
	1    0    0    1   
$EndComp
$Comp
L kbd:kbd_SK6812MINI L4
U 1 1 5C50903B
P 8300 6875
F 0 "L4" H 8300 6875 59  0000 C CNN
F 1 "SK6812MINI" H 8300 6825 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 8300 6875 60  0001 C CNN
F 3 "" H 8300 6875 60  0001 C CNN
	1    8300 6875
	-1   0    0    -1  
$EndComp
$Comp
L kbd:kbd_SK6812MINI L11
U 1 1 5C509042
P 8900 6875
F 0 "L11" H 8900 6875 59  0000 C CNN
F 1 "SK6812MINI" H 8900 6825 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 8900 6875 60  0001 C CNN
F 3 "" H 8900 6875 60  0001 C CNN
	1    8900 6875
	1    0    0    1   
$EndComp
$Comp
L kbd:kbd_SK6812MINI L3
U 1 1 5C509049
P 8300 6375
F 0 "L3" H 8300 6375 59  0000 C CNN
F 1 "SK6812MINI" H 8300 6325 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 8300 6375 60  0001 C CNN
F 3 "" H 8300 6375 60  0001 C CNN
	1    8300 6375
	-1   0    0    -1  
$EndComp
$Comp
L kbd:kbd_SK6812MINI L6
U 1 1 5C509050
P 8900 6375
F 0 "L6" H 8900 6375 59  0000 C CNN
F 1 "SK6812MINI" H 8900 6325 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 8900 6375 60  0001 C CNN
F 3 "" H 8900 6375 60  0001 C CNN
	1    8900 6375
	1    0    0    1   
$EndComp
$Comp
L kbd:kbd_SK6812MINI L2
U 1 1 5C509057
P 8300 5875
F 0 "L2" H 8300 5875 59  0000 C CNN
F 1 "SK6812MINI" H 8300 5825 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 8300 5875 60  0001 C CNN
F 3 "" H 8300 5875 60  0001 C CNN
	1    8300 5875
	-1   0    0    -1  
$EndComp
$Comp
L kbd:kbd_SK6812MINI L1
U 1 1 5C50905E
P 8900 5875
F 0 "L1" H 8900 5875 59  0000 C CNN
F 1 "SK6812MINI" H 8900 5825 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 8900 5875 60  0001 C CNN
F 3 "" H 8900 5875 60  0001 C CNN
	1    8900 5875
	1    0    0    1   
$EndComp
Text Label 7850 5150 0    50   ~ 0
DIN5
Wire Wire Line
	9200 5625 9200 5975
Wire Wire Line
	9200 5975 9200 6475
Wire Wire Line
	9200 6475 9200 6975
Wire Wire Line
	8700 7625 9050 7625
Text Label 9050 7625 0    50   ~ 0
DIN18
Wire Wire Line
	9200 6975 9200 7475
$Comp
L kbd:kbd_SK6812MINI L18
U 1 1 5C54BB48
P 8175 7900
F 0 "L18" H 8175 7900 59  0000 C CNN
F 1 "SK6812MINI" H 8175 7850 16  0000 C CNN
F 2 "kbd:kbd_SK6812MINI" H 8175 7900 60  0001 C CNN
F 3 "" H 8175 7900 60  0001 C CNN
	1    8175 7900
	-1   0    0    -1  
$EndComp
Text Label 7975 7800 2    50   ~ 0
VCC_A
Text Label 7975 8000 2    50   ~ 0
DIN18_A
Text Label 8375 8000 0    50   ~ 0
GND_A
NoConn ~ 8375 7800
Wire Wire Line
	6100 3550 6100 4750
Wire Wire Line
	4700 3550 4700 4750
Wire Wire Line
	8900 2950 8900 4750
Wire Wire Line
	7500 2950 7500 4750
Wire Wire Line
	4850 3875 4850 4150
Wire Wire Line
	6225 4150 6225 3875
Wire Wire Line
	4850 3875 6225 3875
Connection ~ 6225 3875
Wire Wire Line
	7700 4150 7700 3875
Wire Wire Line
	6225 3875 7700 3875
Connection ~ 7700 3875
Wire Wire Line
	7700 3875 9050 3875
Text Label 9050 3875 0    74   ~ 0
GND_B
Text Label 7700 3575 1    74   ~ 0
GND_A
Wire Wire Line
	8700 4150 8400 4150
Wire Wire Line
	10050 3500 9750 3500
Wire Wire Line
	9050 3500 9050 3875
$Comp
L power:GND #PWR0110
U 1 1 5C1BF412
P 15600 950
F 0 "#PWR0110" H 15600 700 50  0001 C CNN
F 1 "GND" H 15600 800 50  0000 C CNN
F 2 "" H 15600 950 50  0001 C CNN
F 3 "" H 15600 950 50  0001 C CNN
	1    15600 950 
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0111
U 1 1 5C1BF418
P 16000 950
F 0 "#PWR0111" H 16000 800 50  0001 C CNN
F 1 "VCC" H 16000 1100 50  0000 C CNN
F 2 "" H 16000 950 50  0001 C CNN
F 3 "" H 16000 950 50  0001 C CNN
	1    16000 950 
	1    0    0    -1  
$EndComp
$Comp
L power:PWR_FLAG #FLG0101
U 1 1 5C1BF41E
P 16000 950
F 0 "#FLG0101" H 16000 1025 50  0001 C CNN
F 1 "PWR_FLAG" H 16000 1100 50  0000 C CNN
F 2 "" H 16000 950 50  0001 C CNN
F 3 "" H 16000 950 50  0001 C CNN
	1    16000 950 
	-1   0    0    1   
$EndComp
$Comp
L power:PWR_FLAG #FLG0102
U 1 1 5C1BF424
P 15600 950
F 0 "#FLG0102" H 15600 1025 50  0001 C CNN
F 1 "PWR_FLAG" H 15600 1100 50  0000 C CNN
F 2 "" H 15600 950 50  0001 C CNN
F 3 "" H 15600 950 50  0001 C CNN
	1    15600 950 
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_01x04 OLED1
U 1 1 5C1C7267
P 14325 7425
F 0 "OLED1" H 14325 7075 50  0000 C CNN
F 1 "I2C_conn" H 14325 7675 50  0000 C CNN
F 2 "kbd:connector_swd" H 14325 7425 50  0001 C CNN
F 3 "" H 14325 7425 50  0001 C CNN
	1    14325 7425
	-1   0    0    1   
$EndComp
$Comp
L power:GND #PWR0121
U 1 1 5C1C72A6
P 15475 7225
F 0 "#PWR0121" H 15475 6975 50  0001 C CNN
F 1 "GND" H 15475 7075 50  0000 C CNN
F 2 "" H 15475 7225 50  0001 C CNN
F 3 "" H 15475 7225 50  0001 C CNN
	1    15475 7225
	0    -1   -1   0   
$EndComp
$Comp
L power:VCC #PWR0123
U 1 1 5C1C72B2
P 15475 7375
F 0 "#PWR0123" H 15475 7225 50  0001 C CNN
F 1 "VCC" H 15475 7525 50  0000 C CNN
F 2 "" H 15475 7375 50  0001 C CNN
F 3 "" H 15475 7375 50  0001 C CNN
	1    15475 7375
	0    1    1    0   
$EndComp
Text GLabel 15325 7525 2    60   Input ~ 0
SCL
Text GLabel 15325 7675 2    60   Input ~ 0
SDA
Wire Wire Line
	14625 7325 14625 7375
Wire Wire Line
	14525 7325 14625 7325
Wire Wire Line
	14575 7425 14575 7525
Wire Wire Line
	14525 7425 14575 7425
Wire Wire Line
	14525 7525 14525 7675
Text GLabel 4700 5150 2    50   Input ~ 0
LED_DATA
Text GLabel 8225 9550 0    50   Input ~ 0
VBAT
Wire Wire Line
	8700 9550 8700 9650
Wire Wire Line
	8700 9950 8700 10050
$Comp
L power:GND #PWR0114
U 1 1 5D295EDB
P 4700 4750
F 0 "#PWR0114" H 4700 4500 50  0001 C CNN
F 1 "GND" H 4700 4600 50  0000 C CNN
F 2 "" H 4700 4750 50  0001 C CNN
F 3 "" H 4700 4750 50  0001 C CNN
	1    4700 4750
	1    0    0    -1  
$EndComp
Connection ~ 4700 4750
$Comp
L power:GND #PWR0127
U 1 1 5D4CC220
P 15150 950
F 0 "#PWR0127" H 15150 700 50  0001 C CNN
F 1 "GND" H 15150 800 50  0000 C CNN
F 2 "" H 15150 950 50  0001 C CNN
F 3 "" H 15150 950 50  0001 C CNN
	1    15150 950 
	1    0    0    -1  
$EndComp
Text GLabel 15150 950  1    50   Input ~ 0
GND
Wire Wire Line
	14525 7675 15125 7675
Wire Wire Line
	14575 7525 15125 7525
Wire Wire Line
	15325 7225 15475 7225
Wire Wire Line
	14625 7375 15125 7375
Wire Wire Line
	15325 7375 15475 7375
$Comp
L Device:Jumper_NO_Small JP2
U 1 1 5C1C726E
P 15225 7225
F 0 "JP2" H 15225 7305 50  0000 C CNN
F 1 " " H 15235 7165 50  0000 C CNN
F 2 "kbd:jumper" H 15225 7225 50  0001 C CNN
F 3 "" H 15225 7225 50  0001 C CNN
	1    15225 7225
	-1   0    0    1   
$EndComp
$Comp
L Device:Jumper_NO_Small JP3
U 1 1 5C1C7275
P 15225 7375
F 0 "JP3" H 15225 7455 50  0000 C CNN
F 1 " " H 15235 7315 50  0000 C CNN
F 2 "kbd:jumper" H 15225 7375 50  0001 C CNN
F 3 "" H 15225 7375 50  0001 C CNN
	1    15225 7375
	-1   0    0    1   
$EndComp
$Comp
L Device:Jumper_NO_Small JP4
U 1 1 5C1C727C
P 15225 7525
F 0 "JP4" H 15225 7605 50  0000 C CNN
F 1 " " H 15235 7465 50  0000 C CNN
F 2 "kbd:jumper" H 15225 7525 50  0001 C CNN
F 3 "" H 15225 7525 50  0001 C CNN
	1    15225 7525
	-1   0    0    1   
$EndComp
$Comp
L Device:Jumper_NO_Small JP5
U 1 1 5C1C7283
P 15225 7675
F 0 "JP5" H 15225 7755 50  0000 C CNN
F 1 " " H 15235 7615 50  0000 C CNN
F 2 "kbd:jumper" H 15225 7675 50  0001 C CNN
F 3 "" H 15225 7675 50  0001 C CNN
	1    15225 7675
	-1   0    0    1   
$EndComp
Connection ~ 14625 7375
Wire Wire Line
	14625 7375 14625 8225
Connection ~ 14575 7525
Wire Wire Line
	14575 7525 14575 8375
Connection ~ 14525 7675
Wire Wire Line
	14525 7675 14525 8525
Wire Wire Line
	14525 8525 15125 8525
Wire Wire Line
	14575 8375 15125 8375
Wire Wire Line
	14625 8225 15125 8225
Wire Wire Line
	14675 7225 15125 7225
Wire Wire Line
	14525 7225 14675 7225
Connection ~ 14675 7225
Wire Wire Line
	14675 7225 14675 8075
Wire Wire Line
	14675 8075 15125 8075
Wire Wire Line
	15325 8375 15475 8375
Wire Wire Line
	15325 8525 15475 8525
Text GLabel 15325 8075 2    60   Input ~ 0
SDA
Text GLabel 15325 8225 2    60   Input ~ 0
SCL
$Comp
L power:VCC #PWR0124
U 1 1 5C1C72B8
P 15475 8375
F 0 "#PWR0124" H 15475 8225 50  0001 C CNN
F 1 "VCC" H 15475 8525 50  0000 C CNN
F 2 "" H 15475 8375 50  0001 C CNN
F 3 "" H 15475 8375 50  0001 C CNN
	1    15475 8375
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0122
U 1 1 5C1C72AC
P 15475 8525
F 0 "#PWR0122" H 15475 8275 50  0001 C CNN
F 1 "GND" H 15475 8375 50  0000 C CNN
F 2 "" H 15475 8525 50  0001 C CNN
F 3 "" H 15475 8525 50  0001 C CNN
	1    15475 8525
	0    -1   -1   0   
$EndComp
$Comp
L Device:Jumper_NO_Small JP9
U 1 1 5C1C729F
P 15225 8525
F 0 "JP9" H 15225 8605 50  0000 C CNN
F 1 " " H 15235 8465 50  0000 C CNN
F 2 "kbd:jumper" H 15225 8525 50  0001 C CNN
F 3 "" H 15225 8525 50  0001 C CNN
	1    15225 8525
	-1   0    0    1   
$EndComp
$Comp
L Device:Jumper_NO_Small JP8
U 1 1 5C1C7298
P 15225 8375
F 0 "JP8" H 15225 8455 50  0000 C CNN
F 1 " " H 15235 8315 50  0000 C CNN
F 2 "kbd:jumper" H 15225 8375 50  0001 C CNN
F 3 "" H 15225 8375 50  0001 C CNN
	1    15225 8375
	-1   0    0    1   
$EndComp
$Comp
L Device:Jumper_NO_Small JP7
U 1 1 5C1C7291
P 15225 8225
F 0 "JP7" H 15225 8305 50  0000 C CNN
F 1 " " H 15235 8165 50  0000 C CNN
F 2 "kbd:jumper" H 15225 8225 50  0001 C CNN
F 3 "" H 15225 8225 50  0001 C CNN
	1    15225 8225
	-1   0    0    1   
$EndComp
$Comp
L Device:Jumper_NO_Small JP6
U 1 1 5C1C728A
P 15225 8075
F 0 "JP6" H 15225 8155 50  0000 C CNN
F 1 " " H 15235 8015 50  0000 C CNN
F 2 "kbd:jumper" H 15225 8075 50  0001 C CNN
F 3 "" H 15225 8075 50  0001 C CNN
	1    15225 8075
	-1   0    0    1   
$EndComp
$Comp
L power:GND #PWR0115
U 1 1 5DDAA4A1
P 16100 3875
F 0 "#PWR0115" H 16100 3625 50  0001 C CNN
F 1 "GND" H 16105 3702 50  0000 C CNN
F 2 "" H 16100 3875 50  0001 C CNN
F 3 "" H 16100 3875 50  0001 C CNN
	1    16100 3875
	1    0    0    -1  
$EndComp
Wire Wire Line
	16100 3175 16100 3875
Connection ~ 16100 3175
Wire Wire Line
	16100 3075 16100 3175
$Comp
L Device:R R6
U 1 1 5DDAA4AA
P 15950 3175
F 0 "R6" V 15835 3175 50  0000 C CNN
F 1 "5.1K" V 15744 3175 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 15880 3175 50  0001 C CNN
F 3 "~" H 15950 3175 50  0001 C CNN
	1    15950 3175
	0    -1   -1   0   
$EndComp
$Comp
L Device:R R5
U 1 1 5DDAA4B0
P 15950 3075
F 0 "R5" V 16157 3075 50  0000 C CNN
F 1 "5.1K" V 16066 3075 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 15880 3075 50  0001 C CNN
F 3 "~" H 15950 3075 50  0001 C CNN
	1    15950 3075
	0    -1   -1   0   
$EndComp
Text GLabel 10575 8375 2    50   Input ~ 0
RX
$Comp
L power:VCC #PWR0117
U 1 1 5DDAD581
P 10550 8050
F 0 "#PWR0117" H 10550 7900 50  0001 C CNN
F 1 "VCC" H 10550 8200 50  0000 C CNN
F 2 "" H 10550 8050 50  0001 C CNN
F 3 "" H 10550 8050 50  0001 C CNN
	1    10550 8050
	0    1    1    0   
$EndComp
$Comp
L Diode:MBR0520 D2
U 1 1 5DDCE829
P 1500 9675
F 0 "D2" H 1500 9459 50  0000 C CNN
F 1 "1N5819" H 1500 9550 50  0000 C CNN
F 2 "Diode_SMD:D_SOD-123" H 1500 9500 50  0001 C CNN
F 3 "" H 1500 9675 50  0001 C CNN
	1    1500 9675
	-1   0    0    1   
$EndComp
$Comp
L Transistor_FET:BSS83P Q1
U 1 1 5DDCE82F
P 2000 9325
F 0 "Q1" H 2200 9400 50  0000 L CNN
F 1 "AO3407" H 2200 9325 50  0000 L CNN
F 2 "Package_TO_SOT_SMD:SOT-23" H 2200 9250 50  0001 L CIN
F 3 "" H 2000 9325 50  0001 L CNN
	1    2000 9325
	1    0    0    -1  
$EndComp
$Comp
L Device:C C1
U 1 1 5DDCE835
P 1800 10025
F 0 "C1" H 1825 10125 50  0000 L CNN
F 1 "10uF" H 1825 9925 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 1838 9875 50  0001 C CNN
F 3 "" H 1800 10025 50  0001 C CNN
	1    1800 10025
	1    0    0    -1  
$EndComp
Wire Wire Line
	1650 9675 1800 9675
Wire Wire Line
	2100 9525 2100 9675
Wire Wire Line
	1800 9875 1800 9675
Connection ~ 1800 9675
Wire Wire Line
	1800 10225 1800 10175
Wire Wire Line
	1800 9325 1150 9325
$Comp
L Device:R R1
U 1 1 5DDCE845
P 1150 10025
F 0 "R1" H 1220 10071 50  0000 L CNN
F 1 "100K" H 1220 9980 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 1080 10025 50  0001 C CNN
F 3 "" H 1150 10025 50  0001 C CNN
	1    1150 10025
	1    0    0    -1  
$EndComp
Connection ~ 1150 9675
Text Notes 1650 8600 0    60   ~ 0
Power and Filtering
Text Notes 5675 8825 0    60   ~ 0
LiPo Charging\n
Wire Wire Line
	1800 9675 2100 9675
Wire Wire Line
	1150 9325 1150 9675
Wire Wire Line
	1150 9675 1350 9675
Wire Wire Line
	1150 9675 1150 9875
$Comp
L Device:C C2
U 1 1 5DDCE854
P 7025 9425
F 0 "C2" H 7140 9471 50  0000 L CNN
F 1 "10uF" H 7140 9380 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric" H 7063 9275 50  0001 C CNN
F 3 "~" H 7025 9425 50  0001 C CNN
	1    7025 9425
	1    0    0    -1  
$EndComp
$Comp
L Device:R R3
U 1 1 5DDCE85A
P 6575 9425
F 0 "R3" V 6782 9425 50  0000 C CNN
F 1 "4.7K" V 6691 9425 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 6505 9425 50  0001 C CNN
F 3 "~" H 6575 9425 50  0001 C CNN
	1    6575 9425
	0    -1   -1   0   
$EndComp
$Comp
L Battery_Management:MCP73831-2-OT U3
U 1 1 5DDCE860
P 5975 9525
F 0 "U3" H 5975 9911 50  0000 C CNN
F 1 "MCP73831" H 5975 10002 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-23-5" H 6025 9275 50  0001 L CIN
F 3 "http://ww1.microchip.com/downloads/en/DeviceDoc/20001984g.pdf" H 5825 9475 50  0001 C CNN
	1    5975 9525
	-1   0    0    1   
$EndComp
$Comp
L power:GND #PWR0120
U 1 1 5DDCE866
P 7025 9775
F 0 "#PWR0120" H 7025 9525 50  0001 C CNN
F 1 "GND" H 7030 9602 50  0000 C CNN
F 2 "" H 7025 9775 50  0001 C CNN
F 3 "" H 7025 9775 50  0001 C CNN
	1    7025 9775
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0125
U 1 1 5DDCE86C
P 6825 9775
F 0 "#PWR0125" H 6825 9525 50  0001 C CNN
F 1 "GND" H 6830 9602 50  0000 C CNN
F 2 "" H 6825 9775 50  0001 C CNN
F 3 "" H 6825 9775 50  0001 C CNN
	1    6825 9775
	1    0    0    -1  
$EndComp
Wire Wire Line
	5975 9125 5975 9225
Wire Wire Line
	6425 9425 6375 9425
Wire Wire Line
	5975 9125 6825 9125
Wire Wire Line
	7025 9775 7025 9575
Wire Wire Line
	1150 10175 1150 10225
Wire Wire Line
	6825 9125 6825 9425
Wire Wire Line
	6725 9425 6825 9425
Connection ~ 6825 9425
Wire Wire Line
	6825 9425 6825 9775
Text GLabel 5000 9825 0    60   Input ~ 0
VBUS
Text GLabel 7175 8925 2    60   Input ~ 0
VBAT
Text GLabel 1050 9325 0    60   Input ~ 0
VBUS
Wire Wire Line
	5475 9625 5575 9625
Text GLabel 8650 8750 2    60   Input ~ 0
BATTERY_PIN
$Comp
L Device:R R2
U 1 1 5DDCE8A6
P 8425 8550
F 0 "R2" H 8355 8504 50  0000 R CNN
F 1 "820K" H 8355 8595 50  0000 R CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 8355 8550 50  0001 C CNN
F 3 "" H 8425 8550 50  0001 C CNN
	1    8425 8550
	-1   0    0    1   
$EndComp
Wire Wire Line
	2100 9125 2450 9125
Wire Wire Line
	1150 9325 1050 9325
Connection ~ 1150 9325
Wire Wire Line
	5475 8925 5475 9625
Wire Wire Line
	5475 8925 7025 8925
Wire Wire Line
	7025 8925 7025 9275
Connection ~ 7025 8925
Wire Wire Line
	7025 8925 7175 8925
Wire Wire Line
	5975 9825 5000 9825
$Comp
L power:GND #PWR0130
U 1 1 5DDCE8DA
P 1800 10225
F 0 "#PWR0130" H 1800 9975 50  0001 C CNN
F 1 "GND" H 1805 10052 50  0000 C CNN
F 2 "" H 1800 10225 50  0001 C CNN
F 3 "" H 1800 10225 50  0001 C CNN
	1    1800 10225
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0131
U 1 1 5DDCE8E0
P 1150 10225
F 0 "#PWR0131" H 1150 9975 50  0001 C CNN
F 1 "GND" H 1155 10052 50  0000 C CNN
F 2 "" H 1150 10225 50  0001 C CNN
F 3 "" H 1150 10225 50  0001 C CNN
	1    1150 10225
	1    0    0    -1  
$EndComp
Wire Wire Line
	10325 8225 10550 8225
$Comp
L kbd:kbd_MJ-4PP-9 J5
U 1 1 5DDAD573
P 10125 8200
F 0 "J5" H 10125 8350 50  0000 C CNN
F 1 "MJ-4PP-9" H 10125 8050 50  0000 C CNN
F 2 "kbd:trrs_connector" H 10400 8375 50  0001 C CNN
F 3 "" H 10400 8375 50  0001 C CNN
	1    10125 8200
	1    0    0    -1  
$EndComp
NoConn ~ 10325 8125
Connection ~ 2100 9675
Text GLabel 1450 1200 1    50   Input ~ 0
VDDH
Text GLabel 2525 9675 2    50   Input ~ 0
VDDH
Text GLabel 1250 1200 1    50   Input ~ 0
DCCH
Text GLabel 1050 2050 0    50   Input ~ 0
VBUS
Text GLabel 1050 3150 0    50   Input ~ 0
S1
$Comp
L jorian-rescue:E73-2G4M08S1C-52840 U1
U 1 1 5D9A1834
P 1950 2500
F 0 "U1" H 1950 1127 50  0000 C CNN
F 1 "E73-2G4M08S1C-52840" H 1950 1036 50  0000 C CNN
F 2 "kbd:E73-2G4M08S1C-52840" H 1950 2500 50  0001 C CNN
F 3 "" H 1950 2500 50  0001 C CNN
	1    1950 2500
	1    0    0    -1  
$EndComp
Text GLabel 1050 2650 0    50   Input ~ 0
S2
Text GLabel 1050 2150 0    50   Input ~ 0
S3
Text GLabel 2250 1200 1    50   Input ~ 0
S4
Text GLabel 2850 2250 2    50   Input ~ 0
S5
Text GLabel 1050 3350 0    50   Input ~ 0
S6
Text GLabel 1050 2750 0    50   Input ~ 0
S7
Text GLabel 1050 2350 0    50   Input ~ 0
S8
Text GLabel 2850 1850 2    50   Input ~ 0
S9
Text GLabel 2850 2450 2    50   Input ~ 0
S10
Text GLabel 1050 3450 0    50   Input ~ 0
S11
Text GLabel 1050 2850 0    50   Input ~ 0
S12
Text GLabel 1050 2550 0    50   Input ~ 0
S13
Text GLabel 2850 2050 2    50   Input ~ 0
S14
Text GLabel 2850 2650 2    50   Input ~ 0
S15
Text GLabel 1050 3550 0    50   Input ~ 0
S16
Text GLabel 1050 2950 0    50   Input ~ 0
S17
Text GLabel 1050 3650 0    50   Input ~ 0
S18
Text GLabel 2850 3050 2    50   Input ~ 0
S19
Text GLabel 2850 3250 2    50   Input ~ 0
S20
Text GLabel 2850 3450 2    50   Input ~ 0
S21
Text GLabel 2850 3650 2    50   Input ~ 0
S22
Text GLabel 1050 1850 0    50   Input ~ 0
RESET
$Comp
L power:GND #PWR0118
U 1 1 5E80C7FF
P 1650 875
F 0 "#PWR0118" H 1650 625 50  0001 C CNN
F 1 "GND" H 1655 702 50  0000 C CNN
F 2 "" H 1650 875 50  0001 C CNN
F 3 "" H 1650 875 50  0001 C CNN
	1    1650 875 
	-1   0    0    1   
$EndComp
Text GLabel 3950 2975 2    50   Input ~ 0
LED_DATA
Text GLabel 1050 2450 0    50   Input ~ 0
D+
Text GLabel 1050 2250 0    50   Input ~ 0
D-
Text GLabel 1050 3050 0    50   Input ~ 0
SWD
Text GLabel 1050 3250 0    50   Input ~ 0
SWC
Text GLabel 3750 2500 0    50   Input ~ 0
P0.06
Text GLabel 3950 2500 2    50   Input ~ 0
P0.07
Text GLabel 3750 2850 0    50   Input ~ 0
P0.08
Text GLabel 3950 2850 2    50   Input ~ 0
P0.12
Text GLabel 3750 3225 0    50   Input ~ 0
P0.04
Text GLabel 3950 3225 2    50   Input ~ 0
P0.04
Wire Wire Line
	3750 3225 3850 3225
Text GLabel 1950 1200 1    50   Input ~ 0
P0.04
Text GLabel 2150 1200 1    50   Input ~ 0
P0.08
Text GLabel 2350 1200 1    50   Input ~ 0
P0.06
Text GLabel 1750 1200 1    50   Input ~ 0
P0.12
Text GLabel 3950 3350 2    50   Input ~ 0
BATTERY_PIN
Wire Wire Line
	3850 3225 3850 3350
Wire Wire Line
	3850 3350 3950 3350
Connection ~ 3850 3225
Wire Wire Line
	3850 3225 3950 3225
Wire Wire Line
	3750 2500 3850 2500
Connection ~ 3850 2500
Wire Wire Line
	3850 2500 3950 2500
Wire Wire Line
	3850 2500 3850 2625
Wire Wire Line
	3850 2625 3950 2625
Wire Wire Line
	3750 2850 3850 2850
Text GLabel 2450 1200 1    50   Input ~ 0
SCL
Text GLabel 2650 1200 1    50   Input ~ 0
SDA
Text GLabel 3950 2625 2    50   Input ~ 0
POWER_PIN
Wire Wire Line
	3850 2850 3850 2975
Wire Wire Line
	3850 2975 3950 2975
Connection ~ 3850 2850
Wire Wire Line
	3850 2850 3950 2850
Text Notes 3575 2075 0    50   ~ 0
top underside pins
Text GLabel 2050 1200 1    50   Input ~ 0
RX
Text Label 11400 7150 2    50   ~ 0
GND
Text Label 12000 7150 0    50   ~ 0
GND_B
Text Label 11400 6450 2    50   ~ 0
GND
Text Label 11400 7050 2    50   ~ 0
LED_VCC
Text Label 11400 7250 2    50   ~ 0
S21
Text Label 11400 5750 2    50   ~ 0
GND
Text Label 12000 5750 0    50   ~ 0
GND_B
NoConn ~ 12000 6950
NoConn ~ 11400 6950
NoConn ~ 11400 6250
NoConn ~ 12000 6250
Text Label 12000 6450 0    50   ~ 0
GND_B
$Comp
L Transistor_FET:BS870 Q2
U 1 1 5E4E7005
P 10575 9800
F 0 "Q2" H 10781 9846 50  0000 L CNN
F 1 "SI2306" H 10781 9755 50  0000 L CNN
F 2 "Package_TO_SOT_SMD:SOT-23" H 10775 9725 50  0001 L CIN
F 3 "http://www.diodes.com/assets/Datasheets/ds11302.pdf" H 10575 9800 50  0001 L CNN
	1    10575 9800
	1    0    0    -1  
$EndComp
Text GLabel 10125 9800 0    60   Input ~ 0
POWER_PIN
$Comp
L Device:R R4
U 1 1 5E4E7016
P 10425 9475
F 0 "R4" V 10632 9475 50  0000 C CNN
F 1 "2M" V 10541 9475 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 10355 9475 50  0001 C CNN
F 3 "~" H 10425 9475 50  0001 C CNN
	1    10425 9475
	0    -1   -1   0   
$EndComp
Text Notes 9450 9675 0    60   ~ 0
Power Control\nNormally On\nLow - Off\n
$Comp
L power:GND #PWR0133
U 1 1 5E644E10
P 10675 10000
F 0 "#PWR0133" H 10675 9750 50  0001 C CNN
F 1 "GND" H 10675 9850 50  0000 C CNN
F 2 "" H 10675 10000 50  0001 C CNN
F 3 "" H 10675 10000 50  0001 C CNN
	1    10675 10000
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0103
U 1 1 5C50901B
P 8600 7625
F 0 "#PWR0103" H 8600 7375 50  0001 C CNN
F 1 "GND" H 8600 7475 50  0000 C CNN
F 2 "" H 8600 7625 50  0001 C CNN
F 3 "" H 8600 7625 50  0001 C CNN
	1    8600 7625
	-1   0    0    -1  
$EndComp
$Comp
L power:GND #PWR0106
U 1 1 5C19B0E3
P 6650 7625
F 0 "#PWR0106" H 6650 7375 50  0001 C CNN
F 1 "GND" H 6650 7475 50  0000 C CNN
F 2 "" H 6650 7625 50  0001 C CNN
F 3 "" H 6650 7625 50  0001 C CNN
	1    6650 7625
	-1   0    0    -1  
$EndComp
$Comp
L power:GND #PWR0105
U 1 1 5C19B0DD
P 5450 7625
F 0 "#PWR0105" H 5450 7375 50  0001 C CNN
F 1 "GND" H 5450 7475 50  0000 C CNN
F 2 "" H 5450 7625 50  0001 C CNN
F 3 "" H 5450 7625 50  0001 C CNN
	1    5450 7625
	-1   0    0    -1  
$EndComp
Text GLabel 11525 9800 2    60   Input ~ 0
LED_VCC
$Comp
L power:GND #PWR0116
U 1 1 5DDAD56D
P 10550 8225
F 0 "#PWR0116" H 10550 7975 50  0001 C CNN
F 1 "GND" H 10550 8075 50  0000 C CNN
F 2 "" H 10550 8225 50  0001 C CNN
F 3 "" H 10550 8225 50  0001 C CNN
	1    10550 8225
	0    -1   -1   0   
$EndComp
Wire Wire Line
	10325 8275 10450 8275
Wire Wire Line
	10450 8275 10450 8375
Wire Wire Line
	10450 8375 10575 8375
Wire Wire Line
	10550 8050 10450 8050
Wire Wire Line
	10450 8050 10450 8175
Wire Wire Line
	10450 8175 10325 8175
$Comp
L power:GND #PWR0134
U 1 1 5E66FD01
P 8700 10050
F 0 "#PWR0134" H 8700 9800 50  0001 C CNN
F 1 "GND" H 8705 9877 50  0000 C CNN
F 2 "" H 8700 10050 50  0001 C CNN
F 3 "" H 8700 10050 50  0001 C CNN
	1    8700 10050
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR0135
U 1 1 5E800E28
P 4000 7300
F 0 "#PWR0135" H 4000 7150 50  0001 C CNN
F 1 "VCC" H 4000 7450 50  0000 C CNN
F 2 "" H 4000 7300 50  0001 C CNN
F 3 "" H 4000 7300 50  0001 C CNN
	1    4000 7300
	0    1    -1   0   
$EndComp
Text GLabel 3500 7300 0    50   Input ~ 0
DCCH
$Comp
L pspice:INDUCTOR X1
U 1 1 5E61AEE9
P 3750 7300
F 0 "X1" H 3750 7119 50  0000 C CNN
F 1 "10uH" H 3750 7210 50  0000 C CNN
F 2 "Inductor_SMD:L_0603_1608Metric" H 3750 7300 50  0001 C CNN
F 3 "~" H 3750 7300 50  0001 C CNN
	1    3750 7300
	1    0    0    -1  
$EndComp
Wire Wire Line
	2100 9675 2525 9675
Text GLabel 4850 5625 1    50   Input ~ 0
LED_VCC
Text GLabel 6050 5625 1    50   Input ~ 0
LED_VCC
Text GLabel 8000 5625 1    50   Input ~ 0
LED_VCC
Text GLabel 9200 5625 1    50   Input ~ 0
LED_VCC
$Comp
L Transistor_FET:BSS83P Q3
U 1 1 5EC3A17E
P 11200 9475
F 0 "Q3" H 11400 9550 50  0000 L CNN
F 1 "AO3407" H 11400 9475 50  0000 L CNN
F 2 "Package_TO_SOT_SMD:SOT-23" H 11400 9400 50  0001 L CIN
F 3 "" H 11200 9475 50  0001 L CNN
	1    11200 9475
	1    0    0    -1  
$EndComp
Wire Wire Line
	11300 9800 11525 9800
Wire Wire Line
	10125 9800 10375 9800
Wire Wire Line
	10675 9600 10675 9475
Wire Wire Line
	10675 9475 10575 9475
Wire Wire Line
	10275 9475 10200 9475
Wire Wire Line
	11300 9675 11300 9800
Wire Wire Line
	10675 9475 11000 9475
Connection ~ 10675 9475
Wire Wire Line
	10200 9125 11300 9125
Wire Wire Line
	11300 9125 11300 9275
Wire Wire Line
	10200 9125 10200 9475
Text GLabel 15800 3075 0    50   Input ~ 0
CC1
Text GLabel 15800 3175 0    50   Input ~ 0
CC2
Wire Wire Line
	1650 875  1650 1200
Text GLabel 1550 1200 1    50   Input ~ 0
P0.07
$Comp
L power:VCC #PWR0101
U 1 1 5EC2EF6B
P 1850 850
F 0 "#PWR0101" H 1850 700 50  0001 C CNN
F 1 "VCC" H 1850 1000 50  0000 C CNN
F 2 "" H 1850 850 50  0001 C CNN
F 3 "" H 1850 850 50  0001 C CNN
	1    1850 850 
	-1   0    0    -1  
$EndComp
Wire Wire Line
	1850 850  1850 1200
Text GLabel 1425 5500 1    50   Input ~ 0
VDDH
Text GLabel 1225 5500 1    50   Input ~ 0
DCCH
Text GLabel 1025 6350 0    50   Input ~ 0
VBUS
Text GLabel 2825 6950 2    50   Input ~ 0
S1
$Comp
L jorian-rescue:E73-2G4M08S1C-52840 U2
U 1 1 5E50BF09
P 1925 6800
F 0 "U2" H 1925 5427 50  0000 C CNN
F 1 "E73-2G4M08S1C-52840" H 1925 5336 50  0000 C CNN
F 2 "kbd:E73-2G4M08S1C-52840" H 1925 6800 50  0001 C CNN
F 3 "" H 1925 6800 50  0001 C CNN
	1    1925 6800
	1    0    0    -1  
$EndComp
Text GLabel 2825 6150 2    50   Input ~ 0
S2
Text GLabel 2225 5500 1    50   Input ~ 0
S3
Text GLabel 1025 6850 0    50   Input ~ 0
S4
Text GLabel 1025 7150 0    50   Input ~ 0
S5
Text GLabel 2825 7350 2    50   Input ~ 0
S6
Text GLabel 2825 6350 2    50   Input ~ 0
S7
Text GLabel 1025 6450 0    50   Input ~ 0
S8
Text GLabel 1025 7250 0    50   Input ~ 0
S10
Text GLabel 2825 7550 2    50   Input ~ 0
S11
Text GLabel 1025 7650 0    50   Input ~ 0
S19
Text GLabel 1025 7750 0    50   Input ~ 0
S20
Text GLabel 1025 7950 0    50   Input ~ 0
S22
Text GLabel 1025 6150 0    50   Input ~ 0
RESET
$Comp
L power:GND #PWR0102
U 1 1 5E50BF25
P 1625 5175
F 0 "#PWR0102" H 1625 4925 50  0001 C CNN
F 1 "GND" H 1630 5002 50  0000 C CNN
F 2 "" H 1625 5175 50  0001 C CNN
F 3 "" H 1625 5175 50  0001 C CNN
	1    1625 5175
	-1   0    0    1   
$EndComp
Text GLabel 1025 6750 0    50   Input ~ 0
D+
Text GLabel 1025 6550 0    50   Input ~ 0
D-
Text GLabel 1025 7350 0    50   Input ~ 0
SWD
Text GLabel 1025 7550 0    50   Input ~ 0
SWC
Text GLabel 1925 5500 1    50   Input ~ 0
P0.04
Text GLabel 2125 5500 1    50   Input ~ 0
P0.08
Text GLabel 2325 5500 1    50   Input ~ 0
P0.06
Text GLabel 1725 5500 1    50   Input ~ 0
P0.12
Text GLabel 2425 5500 1    50   Input ~ 0
SCL
Text GLabel 2625 5500 1    50   Input ~ 0
SDA
Text GLabel 2025 5500 1    50   Input ~ 0
RX
Wire Wire Line
	1625 5175 1625 5500
Text GLabel 1525 5500 1    50   Input ~ 0
P0.07
$Comp
L power:VCC #PWR0108
U 1 1 5E50BF46
P 1825 5150
F 0 "#PWR0108" H 1825 5000 50  0001 C CNN
F 1 "VCC" H 1825 5300 50  0000 C CNN
F 2 "" H 1825 5150 50  0001 C CNN
F 3 "" H 1825 5150 50  0001 C CNN
	1    1825 5150
	-1   0    0    -1  
$EndComp
Wire Wire Line
	1825 5150 1825 5500
Text GLabel 15225 3575 2    50   Input ~ 0
D+
Text GLabel 15225 3375 2    50   Input ~ 0
D-
Text GLabel 15225 3075 2    50   Input ~ 0
CC2
Text GLabel 15225 2975 2    50   Input ~ 0
CC1
Text GLabel 15225 3275 2    50   Input ~ 0
D-
Text GLabel 15225 3475 2    50   Input ~ 0
D+
Text GLabel 15225 2775 2    50   Input ~ 0
VBUS
$Comp
L jorian-rescue:USB_C_Receptacle_USB2.0-Connector J1
U 1 1 5DDAA460
P 14625 3375
F 0 "J1" H 14730 4242 50  0000 C CNN
F 1 "USB-C" H 14730 4151 50  0000 C CNN
F 2 "kbd:usb_type_c" H 14775 3375 50  0001 C CNN
F 3 "https://www.usb.org/sites/default/files/documents/usb_type-c.zip" H 14775 3375 50  0001 C CNN
	1    14625 3375
	1    0    0    -1  
$EndComp
NoConn ~ 15225 3875
NoConn ~ 15225 3975
$Comp
L power:GND #PWR0112
U 1 1 5E5768E3
P 14325 4275
F 0 "#PWR0112" H 14325 4025 50  0001 C CNN
F 1 "GND" H 14330 4102 50  0000 C CNN
F 2 "" H 14325 4275 50  0001 C CNN
F 3 "" H 14325 4275 50  0001 C CNN
	1    14325 4275
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0113
U 1 1 5E58EE0C
P 14625 4275
F 0 "#PWR0113" H 14625 4025 50  0001 C CNN
F 1 "GND" H 14630 4102 50  0000 C CNN
F 2 "" H 14625 4275 50  0001 C CNN
F 3 "" H 14625 4275 50  0001 C CNN
	1    14625 4275
	1    0    0    -1  
$EndComp
Text GLabel 15250 5775 2    50   Input ~ 0
D+
Text GLabel 15250 5575 2    50   Input ~ 0
D-
Text GLabel 15250 5275 2    50   Input ~ 0
CC2
Text GLabel 15250 5175 2    50   Input ~ 0
CC1
Text GLabel 15250 5475 2    50   Input ~ 0
D-
Text GLabel 15250 5675 2    50   Input ~ 0
D+
Text GLabel 15250 4975 2    50   Input ~ 0
VBUS
$Comp
L jorian-rescue:USB_C_Receptacle_USB2.0-Connector J2
U 1 1 5E5FEB08
P 14650 5575
F 0 "J2" H 14755 6442 50  0000 C CNN
F 1 "USB-C" H 14755 6351 50  0000 C CNN
F 2 "kbd:usb_type_c" H 14800 5575 50  0001 C CNN
F 3 "https://www.usb.org/sites/default/files/documents/usb_type-c.zip" H 14800 5575 50  0001 C CNN
	1    14650 5575
	1    0    0    -1  
$EndComp
NoConn ~ 15250 6075
NoConn ~ 15250 6175
$Comp
L power:GND #PWR0129
U 1 1 5E5FEB10
P 14350 6475
F 0 "#PWR0129" H 14350 6225 50  0001 C CNN
F 1 "GND" H 14355 6302 50  0000 C CNN
F 2 "" H 14350 6475 50  0001 C CNN
F 3 "" H 14350 6475 50  0001 C CNN
	1    14350 6475
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0136
U 1 1 5E5FEB16
P 14650 6475
F 0 "#PWR0136" H 14650 6225 50  0001 C CNN
F 1 "GND" H 14655 6302 50  0000 C CNN
F 2 "" H 14650 6475 50  0001 C CNN
F 3 "" H 14650 6475 50  0001 C CNN
	1    14650 6475
	1    0    0    -1  
$EndComp
Text Notes 3175 1600 0    50   ~ 0
Top side layout:\n\n     S1   S2   S3   S4   S5  S19\n\n     S6   S7   S8   S9  S10  S20  S22\n\n     S11 S12  S13  S14 S15  S21 \n\nS18 S16 S17   *
Text GLabel 2825 6550 2    50   Input ~ 0
S12
Text GLabel 1025 6650 0    50   Input ~ 0
S13
Text GLabel 1025 6950 0    50   Input ~ 0
S9
Text GLabel 1025 7850 0    50   Input ~ 0
S21
Text GLabel 1025 7450 0    50   Input ~ 0
S15
Text GLabel 1025 7050 0    50   Input ~ 0
S14
Text GLabel 2825 7950 2    50   Input ~ 0
S18
Text GLabel 2825 6750 2    50   Input ~ 0
S17
Text GLabel 2825 7750 2    50   Input ~ 0
S16
Text Notes 2525 5100 0    50   ~ 0
Underside layout:\n\n      S19  S5   S4   S3   S2   S1\n\nS22  S20  S10  S9   S8   S7   S6\n\n      S21  S15  S14  S13 S12  S11\n\n                        *   S17  S16  S18\n
Text GLabel 2450 9125 2    50   Input ~ 0
BAT_SW
Text GLabel 4075 2250 2    50   Input ~ 0
PGND
Text GLabel 3725 2250 0    50   Input ~ 0
P0.26
Wire Wire Line
	3725 2250 3825 2250
Wire Wire Line
	3975 2250 4075 2250
NoConn ~ 3825 2250
NoConn ~ 3975 2250
Text GLabel 9825 9125 0    50   Input ~ 0
VDDH
Text GLabel 3950 9050 2    60   Input ~ 0
VBAT
$Comp
L Switch:SW_SPDT SW1
U 1 1 5DDCE8AC
P 3750 9050
F 0 "SW1" H 3750 9325 50  0000 C CNN
F 1 "MSK-12C02" H 3750 9225 50  0000 C CNN
F 2 "kbd:switch_MSK-12C02_smd" H 3750 9243 50  0001 C CNN
F 3 "" H 3750 9050 50  0001 C CNN
	1    3750 9050
	-1   0    0    -1  
$EndComp
$Comp
L Switch:SW_SPDT SW2
U 1 1 5F633C0A
P 3800 9900
F 0 "SW2" H 3800 10175 50  0000 C CNN
F 1 "MSK-12C02" H 3800 10075 50  0000 C CNN
F 2 "kbd:switch_MSK-12C02_smd" H 3800 10093 50  0001 C CNN
F 3 "" H 3800 9900 50  0001 C CNN
	1    3800 9900
	-1   0    0    -1  
$EndComp
Text GLabel 3550 9150 0    50   Input ~ 0
BAT_SW
Text GLabel 4000 9900 2    60   Input ~ 0
VBAT
Text GLabel 3600 9800 0    50   Input ~ 0
BAT_SW
Wire Wire Line
	8225 9550 8700 9550
NoConn ~ 2825 7150
NoConn ~ 1325 5500
NoConn ~ 2850 2850
NoConn ~ 1350 1200
NoConn ~ 5575 9425
$Comp
L Device:R R7
U 1 1 5EB5E33A
P 8425 8975
F 0 "R7" H 8355 8929 50  0000 R CNN
F 1 "2M" H 8355 9020 50  0000 R CNN
F 2 "Resistor_SMD:R_0603_1608Metric" V 8355 8975 50  0001 C CNN
F 3 "" H 8425 8975 50  0001 C CNN
	1    8425 8975
	-1   0    0    1   
$EndComp
Wire Wire Line
	8425 8825 8425 8750
Wire Wire Line
	8650 8750 8425 8750
Connection ~ 8425 8750
Wire Wire Line
	8425 8750 8425 8700
$Comp
L power:GND #PWR0104
U 1 1 5EC0A5CB
P 8425 9125
F 0 "#PWR0104" H 8425 8875 50  0001 C CNN
F 1 "GND" H 8430 8952 50  0000 C CNN
F 2 "" H 8425 9125 50  0001 C CNN
F 3 "" H 8425 9125 50  0001 C CNN
	1    8425 9125
	1    0    0    -1  
$EndComp
Text GLabel 8125 8400 0    50   Input ~ 0
BAT_SW
Wire Wire Line
	8125 8400 8425 8400
Wire Wire Line
	10200 9125 9825 9125
Connection ~ 10200 9125
$EndSCHEMATC
